export default function init(el) {
  ...
  inner.classList.add('card-articles-inner');
  const pic = el.querySelector('picture');
  if (pic) {
    const picPara = pic.closest('p');
    if (picPara) {
      const picDiv = document.createElement('div');
      picDiv.className = 'card-articles-picture-container';
      picDiv.append(pic);
  ...
  if (!con) return;
  con.classList.add('card-articles-content-container');
  ...
  ctaPara.classList.add('card-articles-cta-container');
