...
function decorateForeground(fg) {
  ...
  for (const [idx, child] of [...children].entries()) {
    ...
    if (heading) {
      heading.classList.add('hero-platform-heading');
      const detail = heading.previousElementSibling;
      if (detail) {
        detail.classList.add('hero-platform-detail');
      }
    }
    ...
    if (text) {
      ...
      if (idx === 0) {
        child.closest('.hero-platform').classList.add('hero-platform-text-start');
      } else {
        child.closest('.hero-platform').classList.add('hero-platform-text-end');
      }
    }
  }
}
...
export default async function init(el) {
  ...
  const fg = rows.pop();
  fg.classList.add('hero-platform-foreground');
  decorateForeground(fg);
  if (rows.length) {
    const bg = rows.pop();
    bg.classList.add('hero-platform-background');
